...
string* findReleaseLock(int x,int y,int nextX,int nextY,int car){
  ...
  int num =0;
  if(x+1<=row_num && map[x+1][y]==car && !(x+1==nextX && y==nextY)){
    string name="("+to_string(x+1)+","+to_string(y)+")";
    release_lock[num]=name;
    num++;
  }
  if(y+1<=col_num && map[x][y+1]==car && !(x==nextX && y+1==nextY)){
    string name="("+to_string(x)+","+to_string(y+1)+")";
    release_lock[num]=name;
    num++;
  }
  if(x-1>=0 && map[x-1][y]==car && !(x-1==nextX && y==nextY)){
    string name="("+to_string(x-1)+","+to_string(y)+")";
    release_lock[num]=name;
    num++;
  }
  if(y-1>=0 && map[x][y-1]==car && !(x==nextX && y-1==nextY)){
    string name="("+to_string(x)+","+to_string(y-1)+")";
    release_lock[num]=name;
    num++;
  }
...
Guide::StepCode changeNextToDirect(int nowX, int nowY, int nextX,int nextY){
  if(nextX==-1&&nextY==-1)
...
Guide::StepCode Guide::GetNextStep(const CarState &car_state) {
  ...
  if(seq < 0){
    //判断是否存在lua脚本
    redisReply *lua_exit=(redisReply *)redisCommand(redis_context,"SCRIPT %s %s" ,"EXISTS",recover_script_sha.c_str());
    if (NULL == lua_exit){
      Guide::StepCode step_code = ERROR;
      redisFree(redis_context);
      return step_code;
  ...
  int getlock_result=getLock(cur_pos.x,cur_pos.y,car_id,seq,redis_context);
  if(getlock_result != 1){
    redisFree(redis_context);
  ...
  //寻路阶段
  int nextX,nextY;
  findNextStep(cur_pos.x,cur_pos.y,nextX,nextY,car_id,dst_pos.x,dst_pos.y);
  Guide::StepCode step_code=changeNextToDirect(cur_pos.x,cur_pos.y,nextX,nextY);
   
  //放锁
  string* release_lock=findReleaseLock(cur_pos.x,cur_pos.y,nextX,nextY,car_id);
